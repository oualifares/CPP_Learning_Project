...
public:
    ...
    void AirLinesByCompany(const int index)
    {

        if (index < 0 || index >= (int)std::size(airlines))
        {
            throw std::invalid_argument { "Invalide argument : airline index " + index };
        }
        const std::string& tmp = airlines[index];

        const auto nb =
            std::count_if(number_used.begin(), number_used.end(),
                          [&tmp](const std::string& n) { return n.find(tmp) != std::string::npos; });
        std::cout << "number of Aircrafts [" << tmp << "] :" << nb << std::endl;
    }

    size_t getAirlinesSize() { return std::size(airlines); }
};